import {
    BufferGeometry, Vector3, Vector2, Matrix4, Quaternion, CatmullRomCurve3, CircleGeometry,
...
export class Tubular extends BufferGeometry {
    ...
    constructor( pp, tubularSegments, radius, radiusTop, radialSegments, closeTube, closed, geoType, CurveType ){
        ...
        this.closeTube = closeTube || false;
        this.scalar = 1;

        if(this.closeTube) this.generateCap()
    ...
    generateCap(){
        ...
        this.cap = new CircleGeometry( this.radius, this.radialSegments )
        this.cap.rotateX( Math.PI*0.5 )
        let uv = this.cap.attributes.uv
        let ar = uv.array
        let i = uv.count, n
        while(i--){
            n = i*2
    ...
    updateCap( nar ){
        ...
        let pos = this.cap.attributes.position
        let ar = pos.array
        let i = nar.length/3, n, m
    ...
    generateSegment ( i ) {
        ...
        let radiusRatio = (this.radius-this.radiusTop);
        ...
        for ( var j = 0; j <= this.radialSegments; j ++ ) {
            ...
            */
            ...
            let radius =  this.radius - (radiusRatio*point*(1/this.range));
            radius *= this.radiusScale
            if(radius<0) radius = 0.001
            if(!radius) radius = 0.001

            this.vertices[n + n2] =  P.x + radius * this.normal.x * side;
            this.vertices[n + n2 +1] =  P.y + radius * this.normal.y * side;
            this.vertices[n + n2 +2] =  P.z + radius * this.normal.z * side;


            if( i === 0 ){ tmpcap.push(this.vertices[n + n2], this.vertices[n + n2 +1], this.vertices[n + n2 +2]) }
