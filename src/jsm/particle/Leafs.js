...
function Leafs( o ) {
    ...
    var box = o.box || [10,20,10]

    var numParticle = o.numParticle || 10;
    var start = o.start || [ math.rand(-box[0]*0.5, box[0]*0.5 ), box[1], math.rand(-box[2]*0.5, box[2]*0.5) ]
    var dispersion = o.dispersion || [1,1,1];
    var heights = o.heights || [box[1],box[1]*0.5,0];
    ...
    //var g = new THREE.SphereBufferGeometry( o.size || 0.2, 2, 2, 0, 1, 1, 0.5);

    var g = new THREE.PlaneBufferGeometry( 1,2, 2,4);
    ...
    while(j--){
        ...
        y =Math.abs(p[n+1])
        if( y === 1){ p[n]*=0.01;}
        else if( y === 0.5){ if( p[n] === 0) p[n+2]=0.15; else p[n]*=0.75;}
        else { if( p[n] === 0) p[n+2]=0.3; }
    ...
    this.geometry.index = g2.index;
    this.geometry.attributes = g2.attributes;
    ...
    while( i-- ){
        ...
        cc.setHSL( math.rand(0.5, 0.51), math.rand(0.75, 1), math.rand(0.2, 0.4));

        delayDuration.push( math.rand(0, delay), math.rand( delay, duration ) );
        startPosition.push( math.rand(-box[0]*0.5, box[0]*0.5 ), box[1], math.rand(-box[2]*0.5, box[2]*0.5) );
        controlPoint1.push( math.rand(-randomiz*0.3, randomiz*0.3)+Math.cos(angle1)*math.rand(0, dispersion[0]), math.rand(heights[0], heights[1]), math.rand(-randomiz*0.3, randomiz*0.3)+Math.sin(angle1)*math.rand(0, dispersion[0]) );
        controlPoint2.push( math.rand(-randomiz, randomiz)+Math.cos(angle2)*math.rand(0, dispersion[1]), math.rand(heights[1], heights[2]), math.rand(-randomiz, randomiz) + Math.sin(angle2)*math.rand(0, dispersion[1]) );
    ...
    this.geometry.setAttribute('sizer', new THREE.InstancedBufferAttribute( new Float32Array(sizer), 1 ) );
...
Leafs.prototype = Object.assign( Object.create( THREE.Mesh.prototype ), {
