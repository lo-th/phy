...
import { Num, Max, getType, getArray, initArray } from '../core/Config.js';
...
//import initJolt from '../libs_physics/X_JoltM.js';
...
export class engine {
	...
	static initWorld () {
		...
		//settings.set_mMaxWorkerThreads(3)
		//settings.set_mMaxBodies(4096); // Use your config or default to 4096
    ...
    static defineGroup( settings ) {
    	const GROUP_DYNAMIC = 1;
		const GROUP_STATIC = 2;
		const GROUP_FLOOR2 = 4;
		const GROUP_FLOOR3 = 8;
		const GROUP_ALL = GROUP_STATIC | GROUP_DYNAMIC | GROUP_FLOOR2 | GROUP_FLOOR3;
		...
		bpInterface.ConfigureLayer(BP_LAYER_DYNAMIC, GROUP_DYNAMIC | GROUP_FLOOR2 | GROUP_FLOOR3, 0); // Anything that has one of the floor bits set goes into the dynamic broadphase layer
