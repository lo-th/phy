...
let move_speed = 0;     //movement speed
let speed_multiple = 1; //movement speed multiple
const spot_turn_speed = 4;
const leg_move_speed = 8;
const body_move_speed = 3;
...
export class SpiderRobot {
    ...
    action_cmd () {
        ...
        switch (this.action) {
           ...
           // console.log("Turn left");
            ...
            //console.log("Turn right");
            if (!this.is_stand()) this.stand();
            this.turn_right(step);
            break;
            case W_STAND_SIT:
            //console.log("1:up,0:dn");
            if ( step ) this.stand();
            else this.sit();
    ...
    */
    ...
    cartesian_to_polar( x, y, z ) {
        ...
        v = w - length_c;
        this.alpha = Math.atan2(z, v) + Math.acos((Math.pow(length_a, 2) - Math.pow(length_b, 2) + Math.pow(v, 2) + Math.pow(z, 2)) / 2 / length_a / Math.sqrt(Math.pow(v, 2) + Math.pow(z, 2)));
        this.beta = Math.acos((Math.pow(length_a, 2) + Math.pow(length_b, 2) - Math.pow(v, 2) - Math.pow(z, 2)) / 2 / length_a / length_b);
        //calculate x-y-z degree
        this.gamma = (w >= 0) ? Math.atan2(y, x) : Math.atan2(-y, -x);
        //trans degree pi->180
        /*alpha = alpha / pi * 180;
        beta = beta / pi * 180;
        gamma = gamma / pi * 180;*/

        this.alpha = Math.round( this.alpha * todeg )
        this.beta = Math.round( this.beta * todeg ) 
        this.gamma = Math.round( this.gamma * todeg ) 
    }
    ...
    polar_to_servo( leg ) {
        ...
        this.beta = 90 - this.beta;
        this.gamma -= 90;

        //this.beta = 90 - this.beta;
        //this.gamma = 180 -this.gamma;
        ...
        */
        ...
        servo[leg] = this.alpha
        servo[leg+4] = this.beta;
        servo[leg+8] = this.gamma;
