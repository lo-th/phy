...
const map_fragment = THREE.ShaderChunk.map_fragment.replace(
    ...
    if(enableESL) diffuseColor *= lightnessFactor * sampledDiffuseColor * vec4(lightMapClr, 1.);
...
const lights_pars_begin = THREE.ShaderChunk.lights_pars_begin.replace(
    ...
    if(enableESL) irradiance = mix( ambientLightColor, aoColor * 15.0 * ((aoMapClr - 1.0) * 0.28 + 1.0), 1.0 - aoMapClr );
    else irradiance = ambientLightColor;
    `
)
...
export function EnhanceShaderLighting( shader,
    ...
    shader.defines.ENHANCE_LIGHTING = ""
