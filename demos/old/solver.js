...
createScissorLift = () => {
    ...
    for (let j=0; j < 2; ++j){
		...
		for (let i=0; i < linkHeight; ++i){
			...
			let leftAnchorLocation = math.fromTransform( currPosL, currQL, [ pos[0], pos[1]+sinAng*(2 * i), pos[2]-0.9 ], false, true );
			...
			let rightAnchorLocation = math.fromTransform( currPosR, currQR, [ pos[0], pos[1]+sinAng*(2 * i), pos[2]+0.9 ], false, true );
